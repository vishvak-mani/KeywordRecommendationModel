...
def get_list_of_files_from_folder(input_folder):
    ...
    for dirpath, _, filenames in os.walk(input_folder):
        for f in filenames:
            if f.endswith(".pdf") or f.endswith(".doc") or f.endswith(".docx") or f.endswith(".txt") or f.endswith(
                    ".csv"):
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('--stopwords-dir', dest='stopwords_dir', required=False,
                            help='<Optional> Optional File path to folder containing files with to stopwords')
